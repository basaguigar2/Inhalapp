...
public class UpdateAsthmaController implements Initializable{
    ...
    public void checkAsthmaControlled(ActionEvent event) throws SQLException, NotBoundException {
        ...
        if((symptoms_yes.isSelected()||symptoms_no.isSelected())) {
            ...
            patientmanager.editPatient(selectedPatient.getMedical_card_number(), selectedPatient.getPatientName(), selectedPatient.getPatientAge(), selectedPatient.getPatientGender(), selectedPatient.isPregnant(), selectedPatient.isSmoker(), selectedPatient.isSymptoms_controlled(), selectedPatient.isHospitalization(), selectedPatient.getRespiratorydisease(), selectedPatient.getTreatment_stage());
            checkTreatmentButtonPushed(event);
        } else {
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setTitle("Missing data");
